...
class PID {
 public:
  PID(const T &variable, const T &set_point) : variable_(variable), set_point_(set_point) {}
...
template <typename T>
void PID<T>::set_param(double Kp, double Ki, double Kd) {
  std::tie(Kp_, Ki_, Kd_) = std::tie(Kp_, Ki, Kd);
}

template <typename T>
T PID<T>::operator()() const {
